...
Java_realcool_android_OCRNative_init(
...
Java_realcool_android_OCRNative_release(
...
Java_realcool_android_OCRNative_detect(JNIEnv *env, jobject thiz,jobject temp_img, jobject origin_img) {
    Mat temp, origin;
    bitmap_mat(env, temp_img, temp);
    bitmap_mat(env, origin_img, origin);
    std::vector<int> res = detect(temp, origin);
    return cpp_array_to_jintarray(env, res.data(), res.size());
}
